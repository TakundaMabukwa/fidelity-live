...
import { ArrowLeft, Search, Package, Car, CheckCircle } from "lucide-react"
...
export default function LogisticsDashboard() {
  ...
  const [assignmentStep, setAssignmentStep] = useState<"driver" | "crew" | "complete">("driver")
  
  // Use real data from contexts
  const { vehicles: realVehicles, loading: vehiclesLoading, loadVehicles } = useVehicles()
  const { drivers: realDrivers, loading: driversLoading, loadDrivers } = useDrivers()
  const { staff: realStaff, loading: staffLoading, loadStaff } = useStaff()
  
  // Convert real data to component format
  const [vehicles, setVehicles] = useState<Vehicle[]>([])
  const [drivers, setDrivers] = useState<Driver[]>([])
  const [crew, setCrew] = useState<Crewman[]>([])
  ...
  useEffect(() => {
    if (realVehicles.length > 0) {
      const convertedVehicles = realVehicles.map(vehicle => ({
        ...
        capacity_kg: 40000,
        vehicleInfo: {
          structureName: vehicle.structure_name || 'Unnamed Vehicle',
          registrationNo: vehicle.registration_no || 'N/A',
          fleetNo: vehicle.fleet_no || 'N/A',
        },
  ...
  useEffect(() => {
    if (realDrivers.length > 0) {
      const convertedDrivers = realDrivers.map(driver => ({
        id: driver.no.toString(),
        name: driver.surname || 'Unknown',
        surname: driver.full_names || 'Unknown',
        initial: driver.initial || 'U',
        available: true
      }))
  ...
  useEffect(() => {
    if (realStaff.length > 0) {
      const convertedCrew = realStaff.map(staff => ({
        id: staff.id.toString(),
        name: staff.surname || 'Unknown',
        surname: staff.full_names || 'Unknown',
        initial: staff.initial || 'U',
        available: true
      }))
      setCrew(convertedCrew)
    }
  }, [realStaff])

  // Load data when component mounts
  useEffect(() => {
    if (!vehiclesLoading) loadVehicles()
    if (!driversLoading) loadDrivers()
        ...
        <div className="mx-auto max-w-4xl">
          ...
          <Card className="mb-6">
            ...
            <CardContent className="space-y-4">
              ...
              <Button
                className="bg-gray-800 hover:bg-gray-900 w-full text-white"
                  ...
                  {drivers
                    ...
                    .map((driver) => (
                      <div
                        key={driver.id}
                        onClick={() => {
                          assignDriverToVehicle(driver, selectedVehicle)
                          setAssignmentStep("crew")
                        }}
                        className="flex items-center gap-3 hover:bg-blue-50 p-3 border border-gray-200 hover:border-blue-300 rounded-lg transition-all duration-200 cursor-pointer"
                      >
                        <GripVertical className="w-4 h-4 text-gray-400" />
                        <div className="flex-1">
                          <div className="font-medium text-sm">
                            {driver.name.toUpperCase()} {driver.surname.toUpperCase()}
                          </div>
                          <div className="text-gray-600 text-xs">Available for assignment</div>
                        </div>
                      </div>
                    ))}
                </div>
          ...
          {assignmentStep === "crew" && (
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {crew
                    .filter((crewman) => crewman.available)
                    .map((crewman) => (
                      <div
                        key={crewman.id}
                        onClick={() => {
                          assignCrewToVehicle(crewman, selectedVehicle)
                          setAssignmentStep("complete")
                        }}
                        className="flex items-center gap-3 hover:bg-green-50 p-3 border border-gray-200 hover:border-green-300 rounded-lg transition-all duration-200 cursor-pointer"
                      >
                        <GripVertical className="w-4 h-4 text-gray-400" />
                        <div className="flex-1">
                          <div className="font-medium text-sm">
                            {crewman.name.toUpperCase()} {crewman.surname.toUpperCase()}
                          </div>
                          <div className="text-gray-600 text-xs">Available for assignment</div>
                        </div>
                      </div>
              ...
              <CardContent>
                <div className="py-8 text-center">
                  <div className="mb-4 text-green-600">
                    ...
                    <h3 className="font-semibold text-xl">Vehicle Successfully Assigned!</h3>
                    <p className="mt-2 text-gray-600">
                      {selectedVehicle.vehicleInfo?.structureName} has been assigned to:
                    </p>
                  </div>
                  <div className="space-y-2 mb-6">
                    <div className="flex justify-center items-center gap-2">
                      ...
                      <span className="font-medium">
                        Driver: {selectedVehicle.assignedDriver?.name.toUpperCase()} {selectedVehicle.assignedDriver?.surname.toUpperCase()}
                      </span>
                    </div>
                    <div className="flex justify-center items-center gap-2">
                      <Users className="w-4 h-4 text-green-600" />
                      <span className="font-medium">
                        Crew: {selectedVehicle.assignedCrew?.name.toUpperCase()} {selectedVehicle.assignedCrew?.surname.toUpperCase()}
                      </span>
                    </div>
                  </div>
                  <div className="flex justify-center gap-3">
                    <Button
                      ...
                      onClick={() => {
                        setAssignmentStep("driver")
                        setSelectedVehicle({...selectedVehicle, assignedDriver: undefined, assignedCrew: undefined})
                      }}
                    >
                      Reassign
                    </Button>
            ...
            <div className="gap-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-h-96 overflow-y-auto">
              ...
              {availableItems.map((item) => (
                <div
                  ...
                  className={`p-4 border rounded-lg cursor-move transition-all duration-300 transform ${
                    selectedItem?.id === item.id
                      ? "border-blue-500 bg-blue-50 shadow-xl scale-105 -translate-y-1"
                      : isDragging && draggedItem?.id === item.id
                        ? "opacity-50 scale-90 rotate-3 shadow-2xl"
                        : "border-gray-200 hover:border-blue-300 hover:shadow-lg hover:scale-105 hover:-translate-y-1"
                  }`}
                  style={{
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <GripVertical
                      className={`w-4 h-4 transition-colors duration-200 ${
                        selectedItem?.id === item.id ? "text-blue-500" : "text-gray-400"
                      }`}
                    />
